...
if (isset($_SESSION['user_id'])) {
    ...
    $filter_condition = ($filter == 'admin') ? "AND type = 'admin'" : '';

// Query for transactions involving the current admin user
$transactionSql = "SELECT t.transaction_id, t.sender_id, t.receiver_id, t.status, t.item_id, i.item_name, 
                    ...
                    WHERE (us.type = 'admin' OR ur.type = 'admin')";
    ...
    <form method="post">
        <div class="form-group">
            ...
            <select class="form-control" id="filter" name="filter">
                ...
                <option value="admin" <?php if ($filter == 'admin') echo 'selected'; ?>>Admins Only</option>
            </select>
        </div>
    ...
    $(".decline-button").click(function() {
        ...
        if (confirmation) {
            ...
            $.post("reject_item.php", {
                ...
                // Handle the response from the server
                if (response.success) {
                    alert("Transaction declineed successfully!");
                    // Optionally, you can reload the page to reflect the updated transactions
                    window.location.reload();
                } else {
                    alert("declineing transaction failed!");
                }
            }, "json");
        }
    });
