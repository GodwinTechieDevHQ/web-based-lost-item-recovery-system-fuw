...
<div class="container mt-4">
    ...
    <p class="mb-3"><b>Note:</b> <i>Ensure you refresh the page after claiming an item for the changes to reflect!</i></p>
    <p class="mb-4"><strong>Warning:</strong> Only claim an item when it is in your possession. Verify that the receiver of an item claims it for security purposes!</p>
        ...
        <table class="table table-striped">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    ?>
                    <td><?php echo htmlspecialchars($transactionRow['item_name']); ?></td>
                    <td><?php echo htmlspecialchars($senderName); ?></td>
                    <td><?php echo htmlspecialchars($receiverName); ?></td>
                    <td><?php echo htmlspecialchars($transactionRow['status']); ?></td>
                    <td>
                        <?php
                        ...
                        ?>
                        <button class="btn btn-success claim-button" data-transaction-id="<?php echo $transactionRow['transaction_id']; ?>">Claim Item</button>
                        <?php } ?>
                    </td>
                </tr>
                <?php } ?>
...
$(document).ready(function() {
    $(".claim-button").click(function() {
        ...
        var confirmation = confirm("Are you sure the item is in your possession and you want to claim it?");

        if (confirmation) {
            // Make an AJAX request to claim_item.php
            $.post("claim_item.php", { transaction_id: transactionId }, function(response) {
                // Handle the response from the server
                if (response.success) {
                    alert("Item claimed successfully!");
                    // Reload the page to reflect the updated transactions
                    window.location.reload();
                } else {
                    alert("Claiming item failed: " + response.message);
                }
            }, "json");
        }
    });
