...
{
    $sql = "SELECT item_id, item_name, item_description, location, item_image, status, date_lost, owner_id FROM lost_items WHERE date_lost >= '$date' AND report_type != 'Security' ORDER BY date_lost DESC";
    if ($category !== "all") {
        $sql = "SELECT item_id, item_name, item_description, location, item_image, status, date_lost, owner_id FROM lost_items WHERE category_id = '$category' AND date_lost >= '$date' AND status != 'found' ORDER BY date_lost DESC";
    }
